...
2. **溢出防护：** {% raw %}由于总伤害 $\text{TotalDamage}(R)$ 可能超过 $ 64$ 位整数 (`long long`) 的上限 ($ 9 \times 10^{18}$)，在 `count_damage` 函数内部，必须对乘法 $a_i \cdot K_i$ 和累加 $\sum \text{Damage}_i$ 进行溢出检查。如果发生溢出，则表明伤害已达到极大值，足以击败 Boss。{% endraw %}
